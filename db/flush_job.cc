...

  {
    ...
    for (MemTable* m : mems_) {
      ...
      printf("nums:%ld, head_key_ptr:%lx\n", nums,
             (uintptr_t)memtables[memtables.size() - 1]->key().data());
      printf("nums:%ld, key1:%ld\n", nums,
             *(uint64_t*)memtables[memtables.size() - 1]->key().data());
...

    {
      ...
      *(uintptr_t*)ptr = mt_buf_head;
      ...
      // meta_
      int meta_size = send_meta(
          &meta_,
          ptr);  // memcpy((FileMetaData*)ptr,&meta_,  sizeof(FileMetaData));
      ptr += meta_size;
      ...
      if (full_history_ts_low != nullptr) {
        *(uint32_t*)ptr = (uint32_t)(*full_history_ts_low).size();
        ...
        if ((uint32_t)(*full_history_ts_low).size() > 0) {
          memcpy(ptr, full_history_ts_low,
                 (uint32_t)(*full_history_ts_low).size());
      ...
      *(uint32_t*)ptr = (uint32_t)existing_snapshots_.size();
      ...
      if ((uint32_t)existing_snapshots_.size() > 0) {
        std::copy(existing_snapshots_.begin(), existing_snapshots_.end(),
                  (SequenceNumber*)ptr);
      ...
      *(SequenceNumber*)ptr = job_context_->GetJobSnapshotSequence();
      ...
      size_t timestamp_size =
          tboptions.internal_comparator.user_comparator()->timestamp_size();
      ...
      *(uint32_t*)ptr = (uint32_t)(cfd_->GetName()).size();
      ptr += sizeof(uint32_t);
      total_size += sizeof(uint32_t);
      strncpy(ptr, (cfd_->GetName()).c_str(),
              (uint32_t)(cfd_->GetName()).size());
      ...
      *(uint64_t*)ptr = FLAGS_env->mmap_export_desc.size();
      ...
      memcpy(ptr, FLAGS_env->mmap_export_desc.data(),
             FLAGS_env->mmap_export_desc.size());
      ...
      //   "memtable_garbage_bytes:%ld packed_number_and_path_id:%ld
      //   file_size:%ld " "largest_seqno:%ld smallest_seqno:%ld\n",
      //   num_input_entries, memtable_payload_bytes, memtable_garbage_bytes,
      //   meta_.fd.packed_number_and_path_id, meta_.fd.file_size,
      //   meta_.fd.largest_seqno, meta_.fd.smallest_seqno);
#endif
