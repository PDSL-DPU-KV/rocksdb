...
void kv_ycsb_init(kv_ycsb_handle *self, const char *propertyfile, uint64_t *record_cnt, uint64_t *operation_cnt,
                  uint32_t *value_size) {
...
enum kv_ycsb_operation kv_ycsb_next(kv_ycsb_handle self, bool is_seq, uint8_t *key, uint8_t *value) {
    ...
    const rocksdb::Unsigned128 &key_128 = rocksdb::Hash128(key_str.c_str(), key_str.size());
    ...
    if (op != YCSB_READ && value) {
        ...
        memcpy(value, values[0].second.c_str(), values[0].second.size());
    }
    return op;
}