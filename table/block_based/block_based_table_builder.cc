...
uint64_t compress_thread_time = 0;
uint64_t compress_thread_do_time = 0;
uint64_t write_thread_time = 0;
uint64_t write_thread_wait_time = 0;
...
struct BlockBasedTableBuilder::Rep {
  ...
  std::vector<BlockBuilder*> data_block_parallel;
  ...
  std::string last_key;
  std::vector<std::string> last_key_parallel;
  const Slice* first_key_in_next_block = nullptr;
  std::vector<const Slice*> first_key_in_next_block_parallel;
  ...
  std::vector<TableProperties> props_parallel;
  ...
  std::vector<FlushBlockPolicy*> flush_block_policy_parallel;
  ...
  Rep(const BlockBasedTableOptions& table_opt, const TableBuilderOptions& tbo,
    ...
    for (uint32_t i = 0; i < 32; ++i) {
      ...
      props_parallel[i].file_creation_time = tbo.file_creation_time;
      props_parallel[i].orig_file_number = tbo.cur_file_num;
      props_parallel[i].db_id = tbo.db_id;
      props_parallel[i].db_session_id = tbo.db_session_id;
      props_parallel[i].db_host_id = ioptions.db_host_id;
      if (!ReifyDbHostIdProperty(ioptions.env, &props_parallel[i].db_host_id)
               .ok()) {
        ROCKS_LOG_INFO(ioptions.logger, "db_host_id property will not be set");
      }

      data_block_parallel[i] =
...
struct BlockBasedTableBuilder::ParallelCompressionRep {
  ...
  std::vector<std::unique_ptr<Keys>> curr_block_keys_parallel;
  ...
  using BlockRepBuffer_parallel = std::vector<std::vector<BlockRep*>>;
  BlockRepBuffer_parallel block_rep_buf_parallel;
  ...
  BlockRep* PrepareBlock_parallel(CompressionType compression_type,
                                  const Slice* first_key_in_next_block,
                                  BlockBuilder* data_block, uint64_t index) {
    BlockRep* block_rep =
        PrepareBlockInternal(compression_type, first_key_in_next_block);
    assert(block_rep != nullptr);
    data_block->SwapAndReset(*(block_rep->data));
    block_rep->contents = *(block_rep->data);
    std::swap(block_rep->keys, curr_block_keys_parallel[index]);
    curr_block_keys_parallel[index]->Clear();
    return block_rep;
  }
  ...
  void EmitBlock_Compression(BlockRep* block_rep, uint64_t index) {
    assert(block_rep != nullptr);
    assert(block_rep->status.ok());
    if (!compress_queue.push(block_rep)) {
      printf("compress_queue push failed!\n");
      std::abort();
      return;
    }
    ...
    //   });
    // }
    block_rep_buf_parallel[index].push_back(block_rep);
  }
...
void BlockBasedTableBuilder::EmitRemainOpts(uint64_t index) {
  Rep* r = rep_;
  assert(r->state != Rep::State::kClosed);
  if (!ok()) return;
  r->pc_rep->EmitBlock_other(index, r->get_offset());
}
...
void BlockBasedTableBuilder::set_props(uint64_t index, uint64_t num_entries,
...
void BlockBasedTableBuilder::Flush_dpa(uint64_t index, uint64_t key_nums,
                                       uint64_t key_size, uint64_t buffer_size,
                                       uintptr_t keys_ptr, uintptr_t buffer_ptr,
                                       const Slice first_key_in_next_block) {
  Rep* r = rep_;
  assert(r->state != Rep::State::kClosed);
  if (!ok()) return;
  ...
  block_rep = r->pc_rep->PrepareBlock_parallel(
      r->compression_type, r->first_key_in_next_block_parallel[index],
      r->data_block_parallel[index], index);
...
void BlockBasedTableBuilder::Flush_parallel(uint64_t index) {
  ...
  if (r->IsParallelCompressionEnabled() &&
    ...
    r->data_block_parallel[index]->Finish();
    // fprintf(stderr, "r->first_key:%lx",
    // r->first_key_in_next_block_parallel[index]->data()); fflush(stderr); if
    // (index) printf("before prepareblock!\n");
    block_rep = r->pc_rep->PrepareBlock_parallel(
        r->compression_type, r->first_key_in_next_block_parallel[index],
        r->data_block_parallel[index], index);
    // if (index)
    ...
    if (index == 0) {
      ...
      r->pc_rep->EmitBlock_Compression(block_rep, index);
      // printf("index:%lu, after emitblock!\n", index);
    }
  } else {
    // 不支持其他类型操作
...
void BlockBasedTableBuilder::Add_parallel(const Slice& key, const Slice& value,
                                          uint64_t index) {
  Rep* r = rep_;
  assert(rep_->state != Rep::State::kClosed);
  if (!ok()) return;
  ...
  if (IsValueType(value_type)) {
    auto should_flush =
        r->flush_block_policy_parallel[index]->Update(key, value);
    if (should_flush) {
  ...
  r->data_block_parallel[index]->AddWithLastKey(key, value,
                                                r->last_key_parallel[index]);
...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (!r->persist_user_defined_timestamps) {  // 默认不会进入，暂时不管
...
void BlockBasedTableBuilder::BGWorkCompression(
  ...
  while (rep_->pc_rep->compress_queue.pop(block_rep)) {
    // printf("compress queue size: %lu\n",
    // rep_->pc_rep->compress_queue.Size());
...
void BlockBasedTableBuilder::BGWorkWriteMaybeCompressedBlock() {
  ...
  while (r->pc_rep->write_queue.pop(slot)) {
    ...
    write_thread_do_time +=
        std::chrono::duration_cast<std::chrono::nanoseconds>(b3 - a3).count();
  }
  auto b = std::chrono::high_resolution_clock::now();
  write_thread_time +=
      std::chrono::duration_cast<std::chrono::nanoseconds>(b - a).count();
...
// Status BlockBasedTableBuilder::Finish_parallel() {
//   Rep* r = rep_;
//   assert(r->state != Rep::State::kClosed);
//   if (r->IsParallelCompressionEnabled()) {
