...
namespace ROCKSDB_NAMESPACE {
  ...
  struct BlockBasedTableBuilder::Rep {
    ...
    Rep(const BlockBasedTableOptions& table_opt, const TableBuilderOptions& tbo,
      ...
      for (int i = 0;i < compression_opts.parallel_threads;++i) {
        ...
        if (!ReifyDbHostIdProperty(ioptions.env, &props_parallel[i].db_host_id).ok()) {
          ROCKS_LOG_INFO(ioptions.logger, "db_host_id property will not be set");
        }

        data_block_parallel[i] = new BlockBuilder(table_options.block_restart_interval,
                 table_options.use_delta_encoding,
                 false /* use_value_delta_encoding */,
                 tbo.internal_comparator.user_comparator()
                         ->CanKeysWithDifferentByteContentsBeEqual()
        ...
        flush_block_policy_parallel[i] = table_options.flush_block_policy_factory->NewFlushBlockPolicy(table_options, *data_block_parallel[i]);
      }
  ...
  BlockBasedTableBuilder::~BlockBasedTableBuilder() {
    ...
    auto r = rep_;
    for (int i = 0;i < r->compression_opts.parallel_threads;++i) {
  ...
  void BlockBasedTableBuilder::Add_parallel(const Slice& key, const Slice& value, uint64_t index) {
    Rep* r = rep_;
    assert(rep_->state != Rep::State::kClosed);
    if (!ok()) return;
    ...
    if (IsValueType(value_type)) {
      auto should_flush = r->flush_block_policy_parallel[index]->Update(key, value);
      if (should_flush) {
