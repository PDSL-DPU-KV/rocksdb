...
__dpa_global__ void dpa_flush(uintptr_t ctx_ptr) {
    ...
    LOG_DBG("rank:%u, bufferhandle:%lx, node_head:%lx\n", rank, t->bufarr_handle, t->node_head);
    ...
    // 第二种内存构造，用于最后结束
    // | flag(uitn64) | 三个 prop (uint64) | smallest/largeset sqn (uint64_t) | smallest/largest key(slice) |
    ...
    while (cur_addr != tail_addr) {
        ...
        if (!drop) {
            ...
            if (estimate + key.size_ + value.size_ + 3 * sizeof(uint32_t) > datablock_size) {
                // 满足一个datablock的条件
                // 在dpa的栈里面中转一下key
                LOG_DBG("datablock_nums:%lu, key_nums:%lu, iter_num:%lu\n", datablock_nums, key_nums, iter_num);
    ...
    LOG_DBG("iter_num:%lu, drop_num:%lu\n", iter_num, drop_num);
    ...
    *(uint64_t*)buf = 2; // 值为 2 的终止条件
    ...
    LOG_TRACE("after reschedule!\n");

    // if (t->notify_done == 0) {
    //     doca_dpa_dev_sync_event_post_wait_gt(t->aops_handle, t->s_handle, t->call_counter);
    //     ++t->call_counter;
    //     doca_dpa_dev_thread_reschedule();
    // }
    // else {
