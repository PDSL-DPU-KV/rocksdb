...
#ifdef TASKLIST
  node_head = new task_node{std::function<void()>(), nullptr, 0,
      ...
      for (;;) {
...

        {
          ...
          this->condition.wait(lock,
                               [this] { return this->stop || !this->empty(); });
...
// don't allow enqueueing after stopping the pool
    ...
    if (stop) throw std::runtime_error("enqueue on stopped ThreadPool");
    // 生成随机整数
    int priority = *(int*)(std::get<0>(std::forward_as_tuple(
        std::forward<Args>(args)...)));  // std::rand() % 100 + 1;  // int
                                         // priority = v/h      -(p-1)*v/k;
    // int priority = *(int*)char_pointer;
    task_node* add_task =
        new task_node{[task]() { (*task)(); }, nullptr, priority,
                      std::chrono::high_resolution_clock::now()};  // TODO
